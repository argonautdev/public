...
if len(matches) < 3:
	...
	Should have at least three in the form of "[update_action][environment][region] your commit message"
	Below is an example.
	[apply][ekscluster1][us-east-1] Added more tags to cluster
	This is also valid.
	Added more tags to cluster [apply][ekscluster1][us-east-1]
	''')
    exit(1)

action, environment, region = matches[0], matches[1], matches[2]
...
print(
    f"Script in going to run 'terraform {action}' on environment {environment}'s region {region}")

# python can't set environment variables to parent shell
lines = []
for name, val in [("ART_INFRA_ACTION", action), ("ART_INFRA_ENVIRONMENT", environment), ("ART_INFRA_REGION", region)]:
